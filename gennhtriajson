...
if (file_exists("results.json")){
	if (copy("results.json","results_old.json")) {
		unlink("results.json");
		require_once('db.php');
		// $result_json = mysql_query("SELECT user_name, tweet_text, latitude, longitude, tweet_date, aggravation FROM DB_table.flu_tweets WHERE UNIX_TIMESTAMP(NOW()) - tweet_date < 864000;");
